/*
  # Chat and Event Functions

  1. Functions
    - `add_chat_message` - Adds a new chat message and returns it with user info
    - `get_chat_messages` - Retrieves chat messages with user names
    - `count_upcoming_events` - Counts events occurring after a given date

  2. Security
    - All functions use SECURITY DEFINER for controlled access
    - Functions operate on existing tables with proper RLS policies
*/
